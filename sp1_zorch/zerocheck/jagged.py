...
def _challenge_limbs(ef) -> int:
    """Base squeezes for one per-round challenge: the extension degree, or 1
    for a base field (the transcript squeezes its own field directly). SP1
    samples each round challenge as one element of the challenge field via
    `sample_ext_element`, so the count tracks that field's degree."""
    try:
        return efinfo(ef).degree
    except ValueError:
        return 1
