...
eq factor enters once, in the final oracle check. It also samples each round
challenge as one extension element (``sample_ext_element`` =
``efinfo(ef).degree`` base squeezes), the same rule the prover's
``sample_challenge`` follows, so the replay is zorch's stock
``CoeffsSumcheckRound(DEGREE, efinfo(ef).degree)`` — no consumer-local round.
