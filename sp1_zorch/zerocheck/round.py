...
def prove_zerocheck(
    ...
    *,
    ...
    Padding soundness: a zero-padded trace's constraint fold vanishes on its
    zero rows, so the summand is already zero past the chip's real height — no
    explicit real-prefix bound is needed (zorch#283 dropped ``prove``'s
    ``num_real`` perf bound; the result is byte-identical without it)."""
...
def prove_multi_chip_zerocheck(
    ...
    *,
    ...
    Padding soundness: every chip's constraint-fold vanishes on its zero rows,
    so the joint summand is already zero past each chip's real height — no
    explicit real-prefix bound is needed (zorch#283 dropped ``prove``'s
    ``num_real`` perf bound; the result is byte-identical without it)."""
