...
class CapClassJitTest(absltest.TestCase):
    ...
    must zero them, and the runtime-empty seam, where a chip the shard never
    exercised rides at height 0 under a nonzero cap (the live reduce's
    last-live-row index would be -1 and leak a term for a constraint nonzero on
    an all-zero row, so it must fall back to the trivial claim identity)."""
    ...
    def test_capped_stage_matches_exact_and_shares_one_compile(self) -> None:
        ...
        def capped_prove(capped_main, nrs):  # type: ignore[no-untyped-def]
            ...
            # pytree-crossable fields (the ZerocheckStage convention).
            return (
                proof.zeta,
                proof.claimed_sum,
                proof.finals,
        ...
        for seed, alpha_rows, lookup_rows in ((10, 5, 3), (20, 7, 2), (30, 0, 3)):
